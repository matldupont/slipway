...
`package.json`'s version, not a sha (sidebar's says `0.1.0`). Adopt then uses step 2's resolver in
...
```
Given sidebar at its current commit, with no manifest ("chore: start from slipway 0.1.0": a version)
...
5. **`/sync-slipway` and adopt** (#18): the skill, `--adopt`, `PL-`/`PD-` in L1 and in the templates, and
   `SLIPWAY.md`/`BOOTSTRAP.md` text. #18 verified adopt, then sync, on a throwaway clone of sidebar. The
   first real `/sync-slipway` run on sidebar, with its PR, is #12's closing evidence. ~M.
