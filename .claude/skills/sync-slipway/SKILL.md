...
|---|---|
| "AGENT.md's Timezone row: port as written, or adapt?" | "Which timezone should deadlines and appetite dates use? Your hosting and decisions say Toronto. Left as the machine's local time, CI runs in UTC, so a deadline day ends at 8 pm Toronto time (7 pm in winter)." |
| "FRAME adds a Tracker column that K1 requires once M1 is active: fill it?" | "Where is the test for 'people will pay $200–400 tonight' tracked? I found #9 'concierge channels'. Is that it?" |
