...
failure: A question gets a ticket somewhere and stays written as unresolved, so nothing says whether work may continue, and the assumption everything was built on is never written down. In sidebar (#9, #14) it happened with this rule in place — the concierge issue was filed while the marker still said NEEDS CLARIFICATION, and RISK-2's test issue had nowhere to go but a note in the Cheapest-test cell, so `pnpm status` could not tell a scheduled test from an untested one.
enforcement:
  status: check
  pointer: k1
---
