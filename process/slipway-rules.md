...
Imported by `CLAUDE.md` with `@process/slipway-rules.md`. Managed by slipway (`dev/ownership.yaml` in
slipway): a sync replaces this file, so put a project's own rules in `CLAUDE.md`, never here.
